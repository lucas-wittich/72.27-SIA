...
def astar_search(heuristic_func, init_state, goal):
    nodes_expanded = 0
    max_frontier = 0
...
def run_astar_with_report(init_state, goal, heuristic_func, heuristic_name, difficulty=""):
    start_time = time.time()
    solution, nodes_expanded, max_frontier, success = astar_search(heuristic_func, init_state, goal)
    end_time = time.time()
    duration = end_time - start_time
    if success:
        print(f"\nRunning A* with heuristic: {heuristic_name} on puzzle: {difficulty}")
...
def run_all():
    ...
    for puzzle in puzzles:
        ...
        man_res = run_astar_with_report(init, goal, manhattan_heuristic, 'Manhattan Distance', puzzle)
        man_res['puzzle'] = puzzle
        mis_res = run_astar_with_report(init, goal, misplaced_heuristic, 'Misplaced Boxes', puzzle)
